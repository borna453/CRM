...
{{--                    x-on:keydown.delete.stop.prevent="if (! $el.disabled) clearState()"--}}
                ...
                >
                    <input
                        ...
                        x-model="displayText"
                        @php($pattern = ! $hasTime ? '[0-9]{2}-[0-9]{2}-[0-9]{4}' : ($hasDate() && $hasTime ? ('[0-9]{2}-[0-9]{2}-[0-9]{4} [0-9]{2}:[0-9]{2}'.($hasSeconds() ? ':[0-9]{2}' : '')) : ('[0-9]{2}:[0-9]{2}:[0-9]{2}'.($hasSeconds() ? ':[0-9]{2}' : ''))))
                        @php($replacePattern = ! $hasTime ? '(\d{2})(\d{2})(\d{4})' : ($hasDate() && $hasTime ? '(\d{2})(\d{2})(\d{4}) (\d{2})(\d{2})'.($hasSeconds() ? ':(\d{2})' : '') : '(\d{2}):(\d{2}):(\d{2})'.($hasSeconds() ? ':(\d{2})' : '')))
                        @php($replaceWith = ! $hasTime ? '$1-$2-$3' : ($hasDate() && $hasTime ? '$1-$2-$3 $4:$5'.($hasSeconds() ? ':$6' : '') : '$1:$2:$3'.($hasSeconds() ? ':$4' : '')))
                        @change="if (! $event.target.value.match(/^{{ $pattern }}$/)) $event.target.value = $event.target.value.replace(/[^0-9]/g, '').replace(/{{ $replacePattern }}/, '{{ $replaceWith }}'); {{ $hasTime ? 'hour = window.dateTimeHandler.extractHour(displayText); minute = window.dateTimeHandler.extractMinute(displayText);' : '' }} {{ $hasTime && $hasSeconds() ? 'second = window.dateTimeHandler.extractSecond(displayText);' : '' }}; setState(dayjs($event.target.value, '{{ convert_date_format($getDisplayFormat())->to('day.js') }}'))"
                        ...
                        @if ($hasDate())
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2 grow">
                                    <button type="button" x-on:click="focusedMonth--" >
                                ...
                                />
                                <button type="button" x-on:click="focusedYear = (parseInt(focusedYear) + 1).toString()" >
                            ...
                            >
                                ...
                                >
                                    ...
                                    'border-2 border-blue-500': dayIsToday(day) && ! dayIsSelected(day) && ! dayIsDisabled(day)
                                    }"
